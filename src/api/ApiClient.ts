...
export class DynastyClient extends ApiBase {
    ...
    constructor(configuration: Config, baseUrl?: string, http?: { fetch(url: RequestInfo, init?: RequestInit): Promise<Response> }) {
        super(configuration);
    ...
    protected processDynastyAll(response: Response): Promise<Dynasty[]> {
        ...
        if (status === 200) {
            ...
            result200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver) as Dynasty[];
            return result200;
            });
        } else if (status === 401) {
            return response.text().then((_responseText) => {
            return throwException("If Authorization header not present, has no value or no valid jwt bearer token", status, _responseText, _headers);
            });
        } else if (status === 403) {
            return response.text().then((_responseText) => {
            return throwException("If user not authorized to perform requested action", status, _responseText, _headers);
            });
...
export class WeatherForecastClient extends ApiBase {
     ...
     * @return Success
     */
    getWeatherForecast(query?: GetWeatherForecastsQuery | undefined): Promise<WeatherForecast[]> {
